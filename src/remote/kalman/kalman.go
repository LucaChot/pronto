...
func NewKalmanFilter1D(initialMu, initialP, Q, R float64) *KalmanFilter1D {
...
func (kf *KalmanFilter1D) Update(mPods int, drop float64) {
	m := float64(mPods)
    if m == 0 {
...
func (kf *KalmanFilter1D) Reserve(mPods int) float64 {
	return float64(mPods) * kf.mu
}

// State returns the current mean-drop estimate and covariance.
func (kf *KalmanFilter1D) State() (mu, P float64) {
	return kf.mu, kf.P
}
